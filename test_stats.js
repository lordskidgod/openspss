// Automated mathematical verification script for Open SPSS Web statistical engine
import {
  calcDescriptives,
  chiSquarePValue,
  ...
  runCorrelations,
  runLinearRegression
} from './src/engine/statisticalEngine.ts';
import { EMPLOYEE_DATA } from './src/sample_data/datasets.ts';
...
console.log(`Mean Salary: $${desc.mean.toFixed(2)} (Expected ~ $39,944.38)`);
console.log(`Std Dev Salary: $${desc.sd.toFixed(2)}`);

if (desc.n !== 80 || Math.abs(desc.mean - 39944.375) > 1) {
...
console.log('Group Stats Table:', tResult.tables[0].rows);
...
console.log('Regression Model Summary:', regResult.tables[0].rows);
console.log('Regression ANOVA:', regResult.tables[1].rows);
